...
3. Commit the release metadata and, from the monorepo root, create a signed
   `codex-conductor-vX.Y.Z` tag. The tag version must match Conductor's package
   version.
...
API token. The trusted publisher must target the `H-Lupercal/agent-tooling`
repository, `release-codex-conductor.yml` workflow, and
`pypi-codex-conductor` environment.
